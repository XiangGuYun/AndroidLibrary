...
/*
http://blog.csdn.net/csdn_aiyang/article/details/73108606
http://blog.csdn.net/lespace/article/details/52145598
 */
public class MainActivity extends AppCompatActivity {

    int[] oldArray = {10, 3, 6, 2, 4, 9, 1, 8, 7, 5};
    ...
    /*
    冒泡排序思路
    数组的大小为x，那么数组中每个数字都要和其它数字比较（x-1）次，如果后面的数字比前面的数字大则进行交换
     */
     ...
     */
    public void sortInsert(View view) {
        int[] newArray = sortInsert(oldArray);
        for (int i = 0; i < newArray.length; i++) {
            Log.d("Test", newArray[i]+"");
        }
    }
    ...
    public void sortSelect(View view) {
        int[] newArray = sortSelect(oldArray);
        for (int i = 0; i < newArray.length; i++) {
            Log.d("Test", newArray[i]+"");
        }
    }
     ...
     */
    public void sortBubble(View view) {
        int[] newArray = sortBubble(oldArray);
        for (int i = 0; i < newArray.length; i++) {
            Log.d("Test", newArray[i]+"");
        }
    }
    ...
    则可以分别对这两部分记录继续进行排序,已达到整个序列有序的目的，本质就是,
    找一个基位(枢轴,分水岭,作用是左边的都比它小,右边的都比它大.可随机,取名base，
    首先从序列最右边开始找比base小的,如果小,换位置,从而base移到刚才右边(比较时比base小)的位置(记为临时的high位),
    这样base右边的都比base大。然后,从序列的最左边开始找比base大的，如果大,换位置,
    从而base移动到刚才左边(比较时比base大)的位置(记为临时的low位),这样base左边的都比base小，
    循环以上两步,直到 low == heigh, 这使才真正的找到了枢轴,分水岭. 返回这个位置,分水岭左边和右边的序列,分别再来递归。
     */
    public void sortQuick(View view) {
        int[] newArray = sortQuick(oldArray);
        for (int i = 0; i < newArray.length; i++) {
            Log.d("Test", newArray[i]+"");
        }
    }
     ...
     */
    public void sortMerge(View view) {
        int[] newArray = sortMerge(oldArray);
        for (int i = 0; i < newArray.length; i++) {
            Log.d("Test", newArray[i]+"");
        }
    }
     ...
     */
    public void sortShell(View view) {
        int[] newArray = sortShell(oldArray);
        for (int i = 0; i < newArray.length; i++) {
            Log.d("Test", newArray[i]+"");
        }
    }
    ...
    把最大的节点(root)与尾节点(最后一个节点,比较小)位置互换，剩下最后的尾节点,
    现在最大,其余的,从第一个元素开始到尾节点前一位,构造大顶堆递归。
     */
    public void sortHeap(View view) {
        heapAdjust(oldArray, 1, 10);
    }
