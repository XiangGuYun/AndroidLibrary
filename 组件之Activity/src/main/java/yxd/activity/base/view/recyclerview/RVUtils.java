package yxd.activity.base.view.recyclerview;
...
import yxd.activity.base.view.recyclerview.divider.DividerItemDecoration;
import yxd.activity.base.view.recyclerview.refresh_add.AddMoreUtil;
...
public class RVUtils {
    ...
    public <T> AddMoreUtil adapter(List<T> list, onBindData data, SetMultiCellView cellView, int...itemLayoutId){
    ...
    private EasyRVAdapter getAdapter(){
        return adapter;
    }
...
/*
 List<String> list = new ArrayList<>();
        ...
        .addMoreList(new AddMoreUtil.UpdateListener() {
            @Override
            public void update(Message msg) {
                list.addAll((List)msg.obj);
            }
        }, new AddMoreUtil.TimeConsumingListener() {
            @Override
            public void time(Subscriber sub) {
                L.d(Thread.currentThread().getName());
                sleep(1000);
                sub.onNext(msg(myItems));
            }
        });
 */
